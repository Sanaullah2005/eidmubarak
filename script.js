...
// set 1 if the backgroung may fade from dark to medium
// set 2 if the background may fade from light to medium
// set 3 if the background may fade from very dark to very light medium
// set 4 if the background may fade from light to very light
// set 5 if the background may fade from dark to very dark

var fade_effect = 4

//What type of gradient sjoud be applied internet explorer 5x or higher?
// Set " none" or "horzontel" or " verticle"
// speed higher=slower
var speed=30
/////////////////////////////////////
//                                 //
...
function fadebg() {
    ...
    if (stepred!=0 || stepgreen!=0 || stepblue!=0) {
        ...
        if (ie5 && gradient_effect!="none") {
            ...
            if (gradient_effect=="verticle") {gradient_effect=0}
            greencol_1 = hexc[Math.floor(newred/16)];
            greencol_2 = hexc[newred%16];
             bluecol_1 = hexc[Math.floor(newgreen/16)];
             bluecol_2 = hexc[newred%16];
              redcol_1 = hexc[Math.floor(newblue/16)];
              redcol_2 = hexc[newblue%16];
              var newcolorComp1 = "#"+redcol_1+redcol_2+greencol_1+greencol_2+bluecol_1+bluecol_2
              document.body.style.filter = "progid.DXImageTransform.Microsoft.Gradient(startColorstr="+newcolorComp1+",endColorstr="+newcolor+"GradientType="+gradient_effect+")"
        }
        else {
